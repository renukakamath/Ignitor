...
compiled_contract_path = 'C:/Users/renuk/OneDrive/Desktop/RISS/python/viswajyothi/Ignitor/node_modules/.bin/build/contracts/Evuniversal.json'
...
def Userregistration():
	...
	if res:
		...
		lid=insert(q)
		r="insert into user values(NULL,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"%(lid,f,l,pl,ph,e,la,lo,v,vd)
		insert(r)
		print(r)
		data['status']="success"
	return str(data)
...
def Userblog():
	data={}
	lid=request.args['log_id']
	q="select * from blog  inner join user using (user_id)  where user_id!=(select user_id from user where login_id='%s')"%(lid)
	res1=select(q)
